...
describe("createBackendProxyHandler", () => {
  ...
  it("should forward Content-Length so the backend can reject over-sized uploads early", async () => {
    const fetchMock = mockFetch(new Response(null, { status: 201 }));
    globalThis.fetch = fetchMock as unknown as typeof fetch;
    ...
    await handler(
      makeRequest("http://app.test/api/upload", {
        method: "PUT",
        headers: { "Content-Type": "audio/webm", "Content-Length": "7" },
        body: "payload",
      }),
      context(["upload"]),
    );
    ...
    const headers = (fetchMock.mock.calls[0][1]! as RequestInit).headers as Headers;
    // Without this the streamed request is framed as chunked and a backend
    // asking for the declared length sees -1, disabling its fast rejection.
    expect(headers.get("Content-Length")).toBe("7");
  });

  it("should relay a 3xx instead of following it, because a streamed body cannot be replayed", async () => {
    const fetchMock = mockFetch(
      new Response(null, { status: 307, headers: { Location: "/api/elsewhere" } }),
    );
    globalThis.fetch = fetchMock as unknown as typeof fetch;
    ...
    const res = await handler(
      makeRequest("http://app.test/api/upload", { method: "POST", body: "payload" }),
      context(["upload"]),
    );
  ...
  it("should pass the request abort signal to the upstream fetch", async () => {
    const fetchMock = mockFetch(new Response(null, { status: 200 }));
    globalThis.fetch = fetchMock as unknown as typeof fetch;
  ...
  it("should drop Content-Encoding and Content-Length when fetch decoded the response body", async () => {
    // `fetch` decompresses transparently but leaves both headers describing the
    // encoded bytes; relaying them yields ERR_CONTENT_DECODING_FAILED.
    const fetchMock = mockFetch(
      new Response("decoded-json", {
        status: 200,
        headers: {
    ...
    const res = await handler(makeRequest("http://app.test/api/me"), context(["me"]));

    expect(res.headers.get("Content-Encoding")).toBeNull();
    expect(res.headers.get("Content-Length")).toBeNull();
    // Headers describing the resource itself survive.
    expect(res.headers.get("Content-Type")).toBe("application/json");
    expect(res.headers.get("ETag")).toBe('"v1"');
    expect(await res.text()).toBe("decoded-json");
  });

  it("should keep Content-Length on an uncompressed ranged response", async () => {
    const fetchMock = mockFetch(
      new Response("chunk", {
        status: 206,
        headers: {
  ...
  it("should not relay hop-by-hop response headers", async () => {
    const fetchMock = mockFetch(
      new Response("body", {
        status: 200,
